...
# ==== This script modifies the original FAB so that the return result is not affacted by the preset radius ==== # 
# ==== DO NOT use targeted version for this attack ====
...
class FABAttackModifiedBase():
    ...
    def attack_single_run(self, x, y=None, use_rand_start=False):
        ...
        """
        assert self.targeted == False, "Do not want target version for this code."
        x_back_up = x.clone()
    ...
    def perturb(self, x, y):
        ...
        with torch.no_grad():
            ...
            # === Add Distance Log to store the best result ===
            # print("Shape Check (ind_to_fool): ", ind_to_fool.shape)
            best_distance = torch.ones_like(ind_to_fool) * 1e12
            if not self.targeted:
                for counter in range(self.n_restarts):
                    if ind_to_fool.numel() != 0:
                        ...
                        # print("Check Distance Shape: ", res.shape)
                        for i in range(res.shape[0]):
                            idx = ind_to_fool[i]
                            if res[i] < best_distance[i]:
                                best_distance[i] = res[i]
                                adv_sample = adv_curr[i]
                                adv[idx] = adv_sample.clone()
                        #adv[ind_to_fool] = adv_curr.clone()
            else:
                raise RuntimeError("Dont want targetted version in this code")

        return adv


class FABAttackModified(FABAttackModifiedBase):
    """
    Fast Adaptive Boundary Attack (Linf, L2, L1)
    https://arxiv.org/abs/1907.02044
